...
$journal_to_issn_map = array_flip($MedAbbr);

$sql = 'SELECT * FROM publication WHERE id="ed09565d27103d473ca5a7816c8e5a86"';


$sql = 'SELECT * FROM no_doi_found WHERE container="Zool. J. Linn. Soc." LIMIT 100';
...
foreach ($data as $row)
{
	...
	{
		if (isset($journal_to_issn_map[$csl->{'container-title'}]))
		{
			$csl->ISSN[0] = $journal_to_issn_map[$csl->{'container-title'}];
		}	
	}
	
	if (isset($csl->author) && !in_array('author', $keys))
	{
		unset($csl->author);
	}
	...
	{
		if (isset($doc->DOI) && count($doc->DOI) == 1)
		{
			echo 'UPDATE publication SET rdmp_doi = "' . strtolower($doc->DOI[0]) . '" WHERE id="' . $doc->id . '";' . "\n";
		}
		echo 'UPDATE `publication` SET rdmp_doi_search=1 WHERE id="' . $doc->id . '";' . "\n";	
	}
